...
ipcRenderer.on("InvoiceFooter", (e, result) => {
  ...
  hTotalDue.textContent = "Customer paid amount";
  ...
  cDueValue.textContent = result[0].paid[0].changeAmount;
  ...
  // totalPaymentValue.textContent = result[0].paid[0].customerPaid;grandWithTax
  totalPaymentValue.textContent =grandWithTax
  ...
  result.map((order) => {
    ...
    tokenNo.textContent = "Receipt no: " + order.orderInfo[0].tokenno+ "," + order.orderInfo[1].tokenno;;

    let saleInVoice = document.createElement("div");
    saleInVoice.className = "col-md-4 saleinvoice";
    // orderr.orderInfo((invoice) => {
    //   console.log("invoice", invoice);
    //   saleInVoice.textContent = "|Order: " + invoice.saleinvoice;
    // })
    saleInVoice.textContent = "|Order: " + order.orderInfo[0].saleinvoice + "," + order.orderInfo[1].saleinvoice;
